   ...
   //Activate current thread
    ...
    public void run() {
        try {
            ...
            logger.warn("Interrupted. Thread exiting.");
            // Do not exit, let shutdown hooks complete.
        } catch (Throwable e) {
            logger.info("Exception caught. Task canceled.", e);
            e.printStackTrace();
            System.exit(1);
        }
    }
	...
	public void executeAndWait(Runnable task) {
		if (amIInDispatcher()) {
            ...
            Future<?> future = scheduledTasks.submit(task);
	...
	{
		scheduledTasks.submit(task);
	}

	public void execute(Runnable task)
	{
		scheduledTasks.execute(task);
	}
	...
	public void setRejectedExecutionHandler(
		...
		scheduledTasks.setRejectedExecutionHandler(rejectedExecutionHandler);	
	}
