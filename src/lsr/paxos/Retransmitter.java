 ...
 */
public final class Retransmitter implements Runnable {
    ...
    private final DelayQueue<InnerRetransmittedMessage> queue =
            new DelayQueue<Retransmitter.InnerRetransmittedMessage>();
     ...
     */
    public Retransmitter(Network network, String name) {
        assert network != null;
    ...
    @Override
    public void run() {
        logger.info("Retransmitter '{}' starting", name);
        try {
            ...
            logger.warn("Retransmitter '{}' closing: {}", name, e);
        }
    }
    ...
    final class InnerRetransmittedMessage implements RetransmittedMessage, Delayed {
        ...
        synchronized void retransmit() {
            ...
            logger.info("Sending message to: {}, in: {}ms", destinations, getDelay(TimeUnit.MILLISECONDS));
            queue.offer(this);
        }
    }

    private final static Logger logger = LoggerFactory.getLogger(Retransmitter.class);
}
