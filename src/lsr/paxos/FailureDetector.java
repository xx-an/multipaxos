...
public class FailureDetector {
    ...
    private void scheduleTask() {
        ...
        if (paxos.isLeader()) {
            //System.out.println("In faluredetetctor, Sending alive ");
        	task = dispatcher.scheduleAtFixedRate(new SendTask(), Priority.High, 0, sendTimeout);
        } else {
        	//System.out.println("In failuredetetctor, Scheduling suspect, replica = " + ProcessDescriptor.getInstance().localId);
        	task = dispatcher.schedule(new SuspectTask(), Priority.Normal, suspectTimeout);
        	//System.out.println("In failuredetetctor, Scheduled suspect, replica = " + ProcessDescriptor.getInstance().localId);
        }
    }
    ...
    private class SendTask implements Runnable {
        public void run() {
            ...
            //System.out.println("Going to send Alive message");
        ...
        public void onMessageReceived(Message message, final int sender) {
            ...
            dispatcher.dispatch(new Runnable() {
                public void run() {
                	...
                	//System.out.println("Handled msg in failure detector, Leader is = " + paxos.getLeaderId() + " View = " + view);
                	//System.out.println("Storage view = " + storage.getView());
                	if (!paxos.isLeader() && view == storage.getView() &&
                        sender == paxos.getLeaderId()) {
                		//System.out.println("Handled msg in failure detector, going to rest timer, Leader is = " + paxos.getLeaderId() + " View = " + view);
                		resetTimerTask();
                    }
                }
            }, Priority.High);
