...
public final class SelectorThread extends Thread {
    ...
    private void processSelectedKeys() {
        ...
        while (it.hasNext()) {
            ...
            if (key.isAcceptable()) {
                ((ClientManager) key.attachment()).handleAccept();
                if (!key.isValid())
                    continue;
            }
            if (key.isReadable()) {
                ((ReaderAndWriter) key.attachment()).handleRead();
                if (!key.isValid())
                    continue;
            }
            if (key.isWritable()) {
                ((ReaderAndWriter) key.attachment()).handleWrite();
                if (!key.isValid())
                    continue;
            }
        }
    ...
    private final static Logger logger = LoggerFactory.getLogger(SelectorThread.class);
}
