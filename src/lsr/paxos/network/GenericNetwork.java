...
public class GenericNetwork extends Network {
	...
	public void sendToOthers(Message message) {
    	BitSet dests = new BitSet(processes.length);
    	dests.set(0, processes.length);
    	dests.clear(pDesc.localId);
        sendMessage(message, dests);
	}

    @SuppressWarnings("unused")
    private final static Logger _logger = LoggerFactory.getLogger(GenericNetwork.class.getCanonicalName());
}
