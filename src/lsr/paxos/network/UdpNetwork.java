...
//        if (logger.isInfoEnabled()) {
//            logger.info("Sending " + message + " to " + destinations);
//        }
	...
	public void sendToOthers(Message message) {
    	BitSet dests = new BitSet(addresses.length);
    	dests.set(0, addresses.length);
    	dests.clear(p.localId);
        sendMessage(message, dests);
	}

    private final static Logger logger = LoggerFactory.getLogger(UdpNetwork.class.getCanonicalName());
}