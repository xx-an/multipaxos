...
public class TcpConnection {
            ...
            try {
                while (true) {
                    ...
                    synchronized (TcpConnection.this) { // Memory barrier for
                                                        // connected variable.
                        if (!connected) {
                        	//System.out.println("Replica  " + replica.getId() + " is not connected");
                            continue;
                        }
                    }
                    ...
                    try {
                        ...
                        System.out.println("Error sending mesage");
                    }
                }
            } catch (InterruptedException e) {
                logger.error("Sender thread has been interupted and stopped.");
            }
        }
    }
        ...
        public void run() {
            while (true) {
                ...
                try {
                    ...
                    logger.error("Receiver thread has been interupted.");
                    break;
                }
                ...
                while (true) {
                    ...
                    try {
                        ...
                        logger.error("Error deserializing msg", e);
                        System.out.println("Going to close rep  " + replica.getId());
                        close();
                        break;
                    }
                    if (logger.isInfoEnabled()) {
                        logger.info("Received [" + replica.getId() + "], message : " + message.toString() +
    ...
    private void connect() throws InterruptedException {
        if (active) {
            ...
            while (true) {
                try {
                    //System.out.println("Trying to connect to replica = " + replica.getId());
                	socket = new Socket();
                    socket.setReceiveBufferSize(128 * 1024);
                    socket.setSendBufferSize(128 * 1024);
                    logger.info("RcvdBuffer: " + socket.getReceiveBufferSize() + ", SendBuffer: " +
                    ...
                    try {
                    	...
                    	//System.out.println("TCP connection with replica " + replica.getId() + " failed");
                    ...
                    output.flush();
                    //System.out.println("Connected to replica = " + replica.getId());
                    ...
                    logger.warn("Error connecting to " + replica, e);
                }
                Thread.sleep(ProcessDescriptor.getInstance().tcpReconnectTimeout);
            }
    ...
    private synchronized void close() {
        if (socket != null && socket.isConnected()) {
            ...
            try {
                ...
                System.out.println("Closed tcp to replica " + this.replica.getId());
            } catch (IOException e) {
