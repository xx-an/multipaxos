...
public class Log {
    ...
    /** Returns read-only access to the log */
    public TreeMap<Integer, ConsensusInstance> getInstances() {
        return instances;
    }
     ...
     * @return new consensus log
     */
    public ConsensusInstance append(int view, byte[] value, int instID) {
    	ConsensusInstance instance;
    	if(instances.containsKey(instID))
    	{
    		instance = instances.get(instID);
    		instance.updateStateFromKnown(view, value);
    	}
    	else
    	{
	    	instance = createInstance(instID, view, value);
	        instances.put(instID, instance);
    	}
    	if((instID + 1) > nextId)
    	{
    		nextId = instID + 1;
     ...
     * @return new consensus instance
     */
    protected ConsensusInstance createInstance(int inst, int view, byte[] value) {
        return new ConsensusInstance(inst, LogEntryState.KNOWN, view, value);
    }
